...
export const queryKeys = {
  ...
  walletAddress: (name: string, mixdepth: number) => ['wallet', name, 'address', mixdepth] as const,
 ...
 */
export function useNewAddress(walletName: string, mixdepth = 0, enabled = true) {
...
export function useUnlockWallet() {
  ...
  return useMutation<UnlockWalletResponse, Error, { walletFileName: string; password: string }>({
    mutationFn: ({ walletFileName, password }) => unlockWallet(walletFileName, password),
    onSuccess: () => {
...
export function useCreateWallet() {
  ...
  return useMutation<CreateWalletResponse, Error, { walletName: string; password: string; walletType?: string }>({
...
export function useStartMaker() {
  ...
  return useMutation<any, Error, { walletName: string; config?: any }>({
...
export function useToggleMaker() {
  ...
  return {
    toggleMaker: async (walletName: string, isRunning: boolean, config?: any) => {
      if (isRunning) {
...
export function useGetNewAddress() {
  ...
  return useMutation<AddressResponse, Error, { walletName: string; mixdepth?: number }>({
    mutationFn: ({ walletName, mixdepth = 0 }) => getNewAddress(walletName, mixdepth),
